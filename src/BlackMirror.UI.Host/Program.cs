    ...
    class Program
    {
        static void Main(string[] arguments)
        {
            ...
            using (SerilogSupport.InitLogger(new Uri(string.Concat(runPath, "serilog.config"))))
            {
                ...
                IConfigReader configReader = ConfigReaderFactory.Create(cache, environment, "UI");
                IUserInterfaceConfiguration uiConfig = new UserInterfaceConfiguration(configReader);

                Host.Run(() => new ServiceApp(uiConfig));
